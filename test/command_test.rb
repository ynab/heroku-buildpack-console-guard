...
class CommandTest < GuardTest
  ...
  def test_sandboxed_consoles_are_refused
    ['--sandbox', '-s'].each do |flag|
      %w[console c].each do |subcommand|
  ...
  def test_rake_code_evaluating_options_are_refused
    [%w[-e 1], %w[--execute 1], %w[-p 1+1], %w[-E 1],
     %w[--execute-print 1], %w[--execute-continue 1], %w[--execute=1]].each do |args|
  ...
  def test_options_that_name_a_path_are_refused
    [%w[-f Rakefile some:task], %w[-r ./payload some:task], %w[-I /app some:task],
     %w[-R /app some:task], %w[-C /app some:task], %w[--require ./payload],
     %w[--rakefile Rakefile]].each do |args|
      assert_denied wrapper('rake', *args), 'allowlisted'
    end
  end
  ...
  def test_options_that_change_which_rakefile_is_found_are_refused
    ...
    [%w[-g some:task], %w[-G some:task], %w[-N some:task], %w[--system some:task],
     %w[--suppress-backtrace x], %w[--no-such-option]].each do |args|
      assert_denied wrapper('rake', *args), 'allowlisted'
    end
  end
  ...
  def test_the_permitted_rake_options_still_work
    [%w[-T], %w[-T db], %w[-Tdb], %w[--tasks=db], %w[-D db], %w[-W some:task], %w[-P],
     %w[-s some:task], %w[-q some:task], %w[-n some:task], %w[-t some:task],
     %w[-v some:task], %w[-V], %w[-A -T], %w[-B some:task], %w[-m some:task],
     %w[-j 4 some:task], %w[-j4 some:task], %w[-X some:task], %w[--trace some:task],
     %w[--trace=stderr some:task], %w[--backtrace some:task], %w[--dry-run some:task],
     %w[--all --tasks], %w[--comments --tasks], %w[--rules], %w[--job-stats some:task],
     %w[--silent some:task], %w[--version]].each do |args|
